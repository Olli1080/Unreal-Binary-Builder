...
public class BuildPipeline : IBuildPipeline
{
        ...
        IProgress<BuildPipelineProgress>? progress = null)
    {
        ...
        {
            _logger.Error(ex, "Engine Build Pipeline encountered a fatal error.", LogCategory.Build);
            progress?.Report(new BuildPipelineProgress { Stage = BuildPipelineStage.Failed, Message = $"Fatal error: {ex.Message}" });
            return false;
        }
    }
    ...
    public async Task<bool> ExecutePluginPipelineAsync(IEnumerable<PluginCardViewModel> pluginQueue, IProgress<BuildPipelineProgress>? progress = null)
    {
        ...
        {
            _logger.Error(ex, "Plugin Build Pipeline encountered a fatal error.", LogCategory.Build);
            progress?.Report(new BuildPipelineProgress { Stage = BuildPipelineStage.Failed, Message = $"Fatal error: {ex.Message}" });
            return false;
        }
    }
